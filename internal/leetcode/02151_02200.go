...
func MinimumTime(s string) int {
	...
	for i := 1; i <= len(s); i++ {
		if s[i-1] == '0' {
			...
			minCostLeft[i] = utils.MinNum(minCostLeft[i-1]+2, i)
		}
		if s[len(s)-i] == '0' {
			minCostRight[len(s)-i+1] = minCostRight[len(s)-i+2]
		} else {
			minCostRight[len(s)-i+1] = utils.MinNum(minCostRight[len(s)-i+2]+2, i)
		}
	}
	res := len(s)
	for i := 0; i < len(s); i++ {
		res = utils.MinNum(res, minCostLeft[i]+minCostRight[i+1])
	}
	return res
}
