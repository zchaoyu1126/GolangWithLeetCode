...
func DivideLeetCode(dividend int, divisor int) int {
	...
	res := 0
	dividend, isMinus1 := common.AbsInt(dividend)
	divisor, isMinus2 := common.AbsInt(divisor)
...
func Trap(height []int) int {
	...
	for i := 0; i < len(height); i++ {
		...
		for len(stack) > 0 && height[i] > height[stack[len(stack)-1]] {
			...
			curWidth := i - left - 1
			curHeight := common.SmallerNumber(height[i], height[left]) - height[top]
...
func Jump(nums []int) int {
	...
	for i := 0; i < len(nums)-1; i++ {
		maxPosition = common.LargerNumber(maxPosition, nums[i]+i)
		if i == end {
			end = maxPosition
			step++
		}
