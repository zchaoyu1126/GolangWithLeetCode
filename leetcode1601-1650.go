...
func MaxDepth(s string) int {
	...
	for i := 0; i < len(s); i++ {
		...
		if s[i] == '(' {
			...
			if prev >= 0 {
				deep[prev] = common.LargerNumber(deep[prev], d+1)
				res = common.LargerNumber(res, deep[prev])
			} else {
				res = common.LargerNumber(res, d)
			}
		}
	}
	fmt.Println(res)
...
func MinimumEffortPath(heights [][]int) int {
	...
	for t := 1; t < m*n; t++ {
		...
		for k, v := range mp[nextNode] {
			...
			if dis[k] == 0xFFFFFFFF {
				dis[k] = common.LargerNumber(dis[nextNode], v)
			} else {
				dis[k] = common.SmallerNumber(dis[k], common.LargerNumber(dis[nextNode], v))
			}
		}
